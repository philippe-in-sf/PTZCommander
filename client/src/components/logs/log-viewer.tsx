...
const levelIcons: Record<string, React.ReactNode> = {
  ...
  debug: <Bug className="h-4 w-4 text-slate-400 dark:text-slate-500" />,
};
...
const levelColors: Record<string, string> = {
  ...
  debug: "text-slate-500 dark:text-slate-400 bg-slate-500/10",
};
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" size="sm" className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white" data-testid="button-open-logs">
          ...
          <DialogTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Select value={category} onValueChange={setCategory}>
                <SelectTrigger className="w-32 h-8" data-testid="select-log-category">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              logs.map((log, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    levelColors[log.level] || "text-slate-500 dark:text-slate-400"
                  )}
                ...
                >
                  ...
                  <span className="text-slate-400 dark:text-slate-500 flex-shrink-0 w-20">
                    {formatTimestamp(log.timestamp)}
                  </span>
                  <span className="text-slate-500 dark:text-slate-600 flex-shrink-0 w-20 uppercase text-xs">
                    [{log.category}]
                  </span>
