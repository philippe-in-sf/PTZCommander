...
const DEFAULT_STATE: AtemState = {
  ...
  inputs: [],
  transition: { style: 0, nextStyle: 0, inTransition: false, position: 0, remainingFrames: 0, mixRate: 30, dipRate: 30, wipeRate: 30, dveRate: 30, previewEnabled: false },
  fadeToBlack: { isFullyBlack: false, inTransition: false, remainingFrames: 0, rate: 30 },
  downstreamKeyers: [],
  upstreamKeyers: [],
...
export default function SwitcherPage() {
  ...
  const [atemState, setAtemState] = useState<AtemState>(DEFAULT_STATE);
  ...
  const handleAtemState = useCallback((message: any) => {
    if (message.type === "atem_state") {
      setAtemState({
        ...
        transition: message.transition || DEFAULT_STATE.transition,
        fadeToBlack: message.fadeToBlack || DEFAULT_STATE.fadeToBlack,
        downstreamKeyers: message.downstreamKeyers || [],
        upstreamKeyers: message.upstreamKeyers || [],
        macroPlayer: message.macroPlayer || DEFAULT_STATE.macroPlayer,
  ...
  useEffect(() => {
    if (switcher && switcher.status === "online") {
      switcherApi.getStatus(switcher.id).then((status) => {
        if (status.connected) setAtemState({ ...DEFAULT_STATE, ...status });
      }).catch(console.error);
    }
  }, [switcher]);
  ...
  const send = (msg: any) => {
    if (ws?.isConnected()) ws.send(msg);
    else toast.error("Not connected to switcher");
  };

  const getInputLabel = (inputId: number) => {
    const input = atemState.inputs.find(i => i.inputId === inputId);
    return input?.shortName || `${inputId}`;
  };

  const displayInputs = atemState.inputs.length > 0
    ? atemState.inputs.filter(i => i.inputId >= 1 && i.inputId <= 20)
    : Array.from({ length: 8 }, (_, i) => ({ inputId: i + 1, shortName: `${i + 1}`, longName: `Input ${i + 1}` }));
